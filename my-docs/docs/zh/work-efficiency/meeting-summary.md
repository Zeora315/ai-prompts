...
将冗长的会议记录快速整理为结构化纪要，并提炼待办事项与责任人。下面提供两个版本，可任选其一使用。
...
- 参会人员：列出参加会议的所有人。
- 会议议程：列出会议的所有主题和讨论点。
- 主要讨论：详述每个议题的讨论内容，主要包括提出的问题、提议、观点等，将每个人的观点按照缩进的格式列出来，并按照不同的人进行分组。
...
- 把会议记录直接粘贴给 AI 即可，无需额外说明。
- 若需要英文纪要，在开头补充「请用英文输出」。
- 追加「以表格形式输出」可获得更规整的行动项清单。
